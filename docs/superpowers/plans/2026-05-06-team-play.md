...
**Conda env:** `uav`. All commands assume `uv sync` and a working directory of the worktree root.
...
```bash
uv sync && python -c "from envs.quidditch.constants import (
...
```bash
uv sync && python scripts/check_env.py
```
...
```bash
uv sync && python scripts/check_team_mjcf.py
```
...
```bash
uv sync && python scripts/check_team_crash.py
```
...
```bash
uv sync && python scripts/check_team_env.py
```
...
```bash
uv sync && python scripts/check_team_tag.py
```
...
```bash
uv sync && wc -l scripts/train_ppo.py
```
...
```bash
uv sync && python -c "import scripts.train_ppo; print('OK train_ppo imports')"
```
...
```bash
uv sync && python scripts/train_team_ppo.py \
...
```bash
uv sync && python scripts/check_team_warm.py \
...
```bash
uv sync && python scripts/eval_team.py \
...
```bash
uv sync && cat templates/training.toml | grep -E '\[(training|env)\.team' || echo "MISSING"
```
...
```bash
uv sync && make train-team-red RUN_NAME=team_red_v1
```
...
All `git` commands in commit blocks use that path. All `uv` / `python` / `make` commands assume the worktree is the working directory and `uv sync` has been run.
