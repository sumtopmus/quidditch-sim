...
def main() -> None:
    ...
    env.reset(seed=42)
    # Disable Red's takeoff grace: pin both drones at z=1.0 throughout.
    ...
    _pin_red(env)  # re-pin after step (forces drift)
    ...
    # Phase 4: blue jumps to 0.5 m (outside).  Expect EXIT.
    _set_blue(env, 0.5)
    rew, info = _step_zero(env)
    _pin_red(env)
